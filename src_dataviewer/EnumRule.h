...
class CEnumRule :public CRule
{
public:
	CEnumRule(void);
	CEnumRule(CEnumRule &anEnumRule);
	~CEnumRule(void);

private:
    int m_iItemNum;     //item number
	vector<int> m_vecItem;  //vector to store items

public:
	int getItemNum();
	int getItem(int iIndex);
	void setItem(int iIndex, int iItem);
	...
	void BubbleSort();  //sort according to x coordinates
