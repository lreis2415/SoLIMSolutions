...
void CInference::setResultExt(CString strResultExt)
{
	m_strResultExt = strResultExt;
}

double CInference::getCellSize()
	{
		return m_dCellSize;
	}

void CInference::setCellSize(double dCellSize)
	{
...
{	
	...
	double xMin = m_pEnvDatabase->m_pTheSetting->getWestMin();
	double xMax = m_pEnvDatabase->m_pTheSetting->getEastMin();
	double yMin = m_pEnvDatabase->m_pTheSetting->getSouthMin();
	double yMax = m_pEnvDatabase->m_pTheSetting->getNorthMin();

	//m_dCellSize = 30;


	int iRowNum = (yMax - yMin)/m_dCellSize;
	int iColNum = (xMax - xMin)/m_dCellSize;
	...
	{  
		...
		else if(pMaskData->getXmin()>xMax || pMaskData->getYmin()>yMax ||pMaskData->getXmin() + pMaskData->getCellSize() * pMaskData->getNumberOfColumns() < xMin ||pMaskData->getYmin() + pMaskData->getCellSize() * pMaskData->getNumberOfRows() < yMin)
		{
			AfxMessageBox("The mask file cannot cover the common area of input the GIS layers. No mask will be used.");
			m_bUseMask = false;
			maskBuf=NULL;
		}
		else
		{
			...
			for(int i=0;i<iRowNum;i++)
				...
				{
					double dx = xMin + m_dCellSize * j + m_dCellSize/2;
					double dy = yMax - m_dCellSize * i - m_dCellSize/2;
					if(fabs(pMaskData->getData(dx,dy)-m_iMaskValue)<VERYSMALL)
	...
	for(int k=0;k<m_pKnowledgeBase->getSoilTypeNum();k++)
	{
		...
		m_pKnowledgeBase->getSoilType(k)->getInferenceResult(simArray,m_pEnvDatabase,m_bUseMask,maskBuf,m_bCreateCheckFile,chkBuf,&(pInferenceView->m_ProgressBar),k,m_pKnowledgeBase->getSoilTypeNum(),m_dCellSize);

		//write the result to 3dr file
		CFileManager* fm = new CFileManager;

		CString strOutputFileName = m_strResultDir+"\\"+m_pKnowledgeBase->getSoilType(k)->getSoilTypeName()+m_strResultExt;//".3dr";
		CThreeDR* output = new CThreeDR;
		output->setNumberOfRows(iRowNum);
		output->setNumberOfColumns(iColNum);
		output->setXmin(xMin);
		output->setYmin(yMin);
		output->setCellSize(m_dCellSize);
		...
		fm->Writeout(output, strOutputFileName, m_strResultExt);
		delete output;			

		//if the user want ti create check file
		if(m_bCreateCheckFile==true)
		{
			strOutputFileName = m_strResultDir+"\\"+m_pKnowledgeBase->getSoilType(k)->getSoilTypeName()+"_chk"+m_strResultExt;//"_chk.3dr";
			output = new CThreeDR;
			output->setNumberOfRows(iRowNum);
			output->setNumberOfColumns(iColNum);
			output->setXmin(xMin);
