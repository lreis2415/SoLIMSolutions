...
private:
	...
	char VarType[STRLEN];   //added by Fei Du
...
public:
	...
	int readHeader(const char* inputFile);
	int readin(const char* inputFile, int aChunk); //return 0 if reach the file end, otherwise 1;
											//if aChunk is zero, Chunk will be nrow;
	int writeout(const char* outputFile, int aChunk); //return 0 if reach the file end, otherwise 1;
												//if aChunk is zero, Chunk will be nrow;
	...
	char* getVariableType();  //added by Fei
	...
	void setDataName(const char* aDataName);
	...
	void setProjection(char *projectionChar);

	void setVariableType(char *aVarType);   //added by Fei
	...
	void setDataEndLoc(int aDataEndLoc);
		...
		bool resample(long nDstXSize, long nDstYSize, double resolution, float* pDataOutput);
	void findNeighborhood(double x, double y, int winSize, std::vector<float>& neighbors);
};
