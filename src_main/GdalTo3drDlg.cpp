		...
		if(poBand)
		{
			...
			if (CE_None == poBand->RasterIO( GF_Read, 0,0, imgWidth, imgHeight, buffFloat, imgWidth,imgHeight, GDT_Float32, 0, 0 ))
			{
				...
				for(int i=0; i<imgHeight; i++)
					...
					{
						if (fabs(buffFloat[i*imgWidth + j] - nodataValue) < VERYSMALL)
                           buffFloat[i*imgWidth + j] = NOFLOATVALUE;
					}
				output->setNullDataValue(NOFLOATVALUE);

				CString dataName = dropFileSuffix(extractFileName(m_OutputFileName));
				output->setDataName(dataName.GetBuffer(1));
