...
//read and write
int CThreeDR::readHeader(const char* inputFile){
...
int CThreeDR::readin(const char* inputFile, int aChunk){ //return 0 if reach the file end, otherwise 1;
	...
	if(inCurLoc == 0){  //First time open the file
		...
		for(int nl=0; nl<NumberOfRecords; nl++){
			...
			else if(strcmp(utilityString, "NoDataValue:")==0){
				fscanf(fp, "%f", &NullDataValue);
			}
			/*else if(strcmp(utilityString, "VariableType:")==0){
				fscanf(fp, "%s", VarType);
			}*/
			else if(strcmp(utilityString, "FirstDataByte:")==0){
				fscanf(fp, "%d", &FirstDataByte);
...
int CThreeDR::writeout(const char* outputFile, int aChunk){ //return 0 if reach the file end, otherwise 1;
	...
	if(outCurLoc == 0){ //first time open the file.
		...
		//if(fabs(NullDataValue - UNLIKELY) > VERYSMALL){
		...
		//if(strlen(VarType)>0)		fprintf(fp, "%s %s\n", "VariableType: ", VarType);
	    ...
	    /*if(fabs(NullDataValue - UNLIKELY)>VERYSMALL)*/  fprintf(fp, "%s %f\n", "NullDataValue: ", NullDataValue);
...
bool CThreeDR::spatialRangeMatch(CThreeDR* anotherThreeDR){
	...
	//modified by Fei in order to loosen the restriction

	if(anotherThreeDR == NULL) return false;
	if(fabs(Xmin-anotherThreeDR->Xmin)<1
		&& fabs(Ymin-anotherThreeDR->Ymin)<1
		&& fabs(CellSize-anotherThreeDR->CellSize)<VERYSMALL)
		return true;
	else
		return false;
...
char* CThreeDR::getVariableType(){
	return VarType;
}

char* CThreeDR::getProjection() {
...
void CThreeDR::setDataName(const char* aDataName){
...
void CThreeDR::setProjection(char *projectionChar) {
	this->Projection = projectionChar;
}
...
void CThreeDR::setVariableType(char* aVarType){
	strcpy(VarType, aVarType);
}
...
bool CThreeDR::resample(long nDstXSize, long nDstYSize, double resolution, float* pDataOutput)
{
	if (nDstXSize == 0 || nDstYSize == 0) return false;

	int nSrcXSize = this->getNumberOfColumns();
	int nSrcYSize = this->getNumberOfRows();

	double scale = resolution / this->getCellSize();
	
	long fX, fY;
	for(long y = 0; y < nDstYSize; y++)
	{
		fY = long( (y + 0.5) * scale);
		for(long x = 0; x < nDstXSize; x++)
		{
			fX = long( (x + 0.5) * scale);
			long iDst = y * nDstXSize + x;
			long iSrc = fY * nSrcXSize + fX;
			pDataOutput[iDst] = this->DataBuf[iSrc];
		}
	}
	return true;
}
...
void CThreeDR::findNeighborhood(double x, double y, int winSize, vector<float>& neighbors)
{
	...
	float nodataValue = this->getNullDataValue();

	// row and column number in the point (x, y)
	int rowCentor = int( nRows - (y - ymin) / cellsize ) - 1; 
	int colCentor = int( (x - xmin) / cellsize );

	if (rowCentor < 0 || rowCentor > this->getNumberOfRows() - 1 
		|| colCentor < 0 || colCentor > this->getNumberOfColumns() - 1)
		return;

	if(abs(this->getDataBuf()[rowCentor*nCols + colCentor] + 9999) < CBRZERO)
		return;
	...
	for (int i = rowBegin; i <= rowEnd; ++i)
	{
		...
		{
			float curVal = this->getDataBuf()[i*nCols + j];
