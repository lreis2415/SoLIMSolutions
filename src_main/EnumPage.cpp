...
void CEnumPage::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Control(pDX, IDC_EDIT_LAYER2, m_editLayerName);
...
void CEnumPage::OnBnClickedButtonBrowseLayerFile()
{
	CMainFrame*   pFrame   =   (CMainFrame*)AfxGetMainWnd();
	CSoLIMDoc *pDoc = (CSoLIMDoc *)pFrame->GetActiveDocument();
	...
	{   
		...
		else 
		{
			...
			pLayer = NULL;
			CString msg;
			msg.Format("Source file of layer %s can not be found, this layer will not be attached",strLayerName);
			AfxMessageBox(msg);
			return;
		}
...
void CEnumPage::OnCbnSelchangeComboLayerSelect()
{
	UpdateData(true);
...
void CEnumPage::OnBnClickedButtonApplyRule()
{
	// TODO: Add your control notification handler code here
