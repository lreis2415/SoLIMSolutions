...
public:
	...
	CMruFile m_CMruFile;         //recent open files

	int m_iProjectType;

	CProjectCase *m_pProjectCase;
    CProjectCase *m_pProjectCaseEdit;


	CConvertTacitProjectDlg m_convertTacitPrjDlg;
	CConvertOldProjectDlg m_convertOldPrjDlg;
	CConvertSIEDlg m_convertSIEDlg;

	CConvert5ProjectDlg m_convert5PrjDlg;

private:
	int m_iCurViewType; //0:DatabaseView 1:KnowledgeView 2:InferenceView 3:CasebaseView 4:CasebaseInferenceView
...
protected:
	...
	DECLARE_MESSAGE_MAP()
public:
	void setCurViewType(int iCurViewType);
	int getCurViewType();
	void NewProject(CString strProjectDir, CString strProjectName, int iProjectType);
	void ReadProject(CString strFileName);

	//void ReadProjectCase(CString strFileName);

	void WriteProject(CString strFileName,CProject *pProject);

	void WriteProjectCase(CString strFileName,CProjectCase *pProjectCase);

	void CleanMemory();
	void OpenMruFile(int nMru);

	void CopyProject(CProject *pProjectFrom, CProject *pProjectTo,bool bIsEditVersion);
	void CopyProjectCase(CProjectCase *pProjectFrom, CProjectCase *pProjectTo,bool bIsEditVersion);


		void SaveKnowledgeBase(CString strFileName, CProject *pProject);

		void LoadKnowledgeBase(CString strFileName, CProject *pProject);
   // void ConvertSoLIM2Project(CString projectFile,CString newProjectName, CString newProjectDir);
	...
	afx_msg void OnProjectConvertfromSoLIM4();
	afx_msg void OnProjectConvertfromoldsolim();
	afx_msg void OnProjectConvertfromsie();




	afx_msg void OnProjectConvertfrosolimsolutions5();
};
