...
FCM::~FCM()
{
	 ...
	 for(int i=0; i<numOfValidPixel; i++)	 
...
void FCM::InitiatePixels()
{
		 ...
		 pixelFeatures = new PixelFeatures[numOfValidPixel];
	 for(int i=0; i<numOfValidPixel; i++)
		 pixelFeatures[i].vars = new float[ NumberOfVariables];
}
...
//		printf("Finish the %d iteration!!!\n", iterationNum);
//		printf("The errMax is %lf:\n", errMax);
...
void FCM::IniMembershipMatrix()
{
			...
			membershipMatrix = new double*[NumberOfClasses];

		updatedMembershipMatrix = new double*[NumberOfClasses];
		...
		{    
			...
			membershipMatrix[i] = new double[numOfValidPixel];
			updatedMembershipMatrix[i] = new double[numOfValidPixel];		
			//updatedMembershipMatrix[i].resize(samplingPixelNum);
		}



	double randomNum; //random number between 0~1.0
	double remainOfMembership;

	double powerValue; //dummy variable

	for(int j=0;j<numOfValidPixel;j++)
	{
		remainOfMembership = 1.0;

		for(int i=0;i<NumberOfClasses-1;i++)
		{
			randomNum = (double)rand()/RAND_MAX; //RAND_MAX is defined in VC library
...
void FCM::UpdateClassCenters()
{
	...
	for(i=0;i<NumberOfClasses;i++)
	{
		for(j=0;j<NumberOfVariables;j++)
		{
			...
			{
				classCentroid[i][j] += pow(membershipMatrix[i][k],WeightExp)*pixelFeatures[k].vars[j];

				dummySum += pow(membershipMatrix[i][k],WeightExp);
			}
...
void FCM::CalEntropy()
{
	...
	for(i=0;i<NumberOfClasses;i++)
	{
		for(j=0;j<samplingPixelNum;j++)
		{
			...
			payoff += pow(membershipMatrix[i][j],WeightExp) * dist;
		}
	}
...
void FCM::WriteMembershipFiles()
{
	...
	for(int i=0;i<NumberOfClasses;i++)
	{
		for(int j=0;j<samplingPixelNum;j++)
		{
			...
			membershipMatrix[i][j] = dummySum1/sum;
		...
		//float *alphaCutBuf = new float[row*col];
	   ...
	   for(int irow=0;irow<nrow;irow++)
		...
		for(int i=0; i<NumberOfClasses; i++)
		{
		  ...
		  for(int m=0; m<samplingPixelNum; m++)
		  {
			  resultBuf[pixelFeatures[m].y*ncol+pixelFeatures[m].x] =membershipMatrix[i][m];
		  }
...
/*		  for(int irow=0;irow<nrow;irow++)
		     for(int icol=0;icol<ncol;icol++)		 	 
		   {
			  ...
			  if(resultBuf[irow*ncol+icol]>alpha)
			  {
				   alphaCutIntegrationBuf[irow*ncol+icol] += 1*(i+1);
			  }
			  else 
				  alphaCutIntegrationBuf[irow*ncol+icol] += 0*(i+1); 
		...
		/*  CString strOutputFileName;
		  strOutputFileName.Format("%s%s", strResultDir,"\\alpha_cut.3dr");
			CThreeDR* output = new CThreeDR;
			output->setNumberOfRows(nrow);
			output->setNumberOfColumns(ncol);
			output->setXmin(m_pEnvDatabase->m_pTheSetting->getWest());
