...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_leftBar.Create(_T("         Knowledge"), WS_VISIBLE, this, 995))
	{
...
{
	  m_strTitle = "SoLIM Expert Knowledge Editor"; 
	  cs.x = 40;
	  cs.y = 40;
	  cs.cx = 900; 
      cs.cy = 700; 
...
void CMainFrame::OnHelp()
{
	...
	int result=(int)(ShellExecute(m_hWnd,_T("open"),_T("SoLIMSolutions.hlp"),NULL,_T(app->m_exePath),SW_SHOW));
	if(result<32)
	{
		if(AfxMessageBox("Help file can not be found! \nDo you want to find it by yourself?",MB_ICONQUESTION|MB_YESNO)==IDYES)
		{
			static char BASED_CODE szFilter[] = "help files (*.hlp)|*.hlp||";
			CFileDialog m_hlpFileDlg(TRUE, NULL, "*.hlp", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);	
...
void CMainFrame::OnClose()
{
	...
	{
		int result;
		result = MessageBox("Save knowlege base file before exiting program?","Save File", MB_YESNOCANCEL);
		if(result==IDYES)
		{
			pDoc->OnProjectSave();
		}
...
void CMainFrame::OnMappingWithSamples()
{
	...
	{
		pApp->pSmallScaleDlg = new CBRSmallScaleDlg();
		pApp->pSmallScaleDlg->Create(IDD_CBR_SEL_DATA, NULL);
	}
