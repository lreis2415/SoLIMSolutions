...
void CInPlaceEdit::OnPaint()
{
	...
	CRect   rc;   
  GetClientRect(&rc);   
  CBrush*   pNewBrush=new   CBrush(RGB(192,   192,   192));//灰色   
  dc.FillRect(&rc,pNewBrush);   
          delete   pNewBrush;  
	// 不为绘图消息调用 CEdit::OnPaint()
}

HBRUSH CInPlaceEdit::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	HBRUSH hbr = CEdit::OnCtlColor(pDC, pWnd, nCtlColor);
     
     pDC->SetBkMode(TRANSPARENT);//这样还可把文字背景设为透明！    
	 hbr   =   CreateSolidBrush(RGB(255,0,0));//hbr为你要设的背景色，   
  return   hbr;//这一句不能少，靠返回的hbr重画的！ 
