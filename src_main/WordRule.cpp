...
CBrush brush;
	...
	if(pWordCurve != NULL)
	{
		for(int i=0;i<pWordCurve->getKnotNum();i++)
		{
				...
				CRect rc((pWordCurve->getKnotX(i)-m_dDispMin)*(cx-40)/(m_dDispMax-m_dDispMin)+20-3, cy-(pWordCurve->getKnotY(i)*(cy-40)+20+3), (pWordCurve->getKnotX(i)-m_dDispMin)*(cx-40)/(m_dDispMax-m_dDispMin)+20+3, cy-(pWordCurve->getKnotY(i)*(cy-40)+20-3));
