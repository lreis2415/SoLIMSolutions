...
// CConvertOldProjectDlg dialog

IMPLEMENT_DYNAMIC(CConvertOldProjectDlg, CDialog)
CConvertOldProjectDlg::CConvertOldProjectDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CConvertOldProjectDlg::IDD, pParent)
	, m_OldPrjDir(_T(""))
	, m_OldPrjFile(_T(""))	
	, m_NewPrjDir(_T(""))
	, m_NewPrjName(_T(""))
...
CConvertOldProjectDlg::~CConvertOldProjectDlg()
{
}

void CConvertOldProjectDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CConvertOldProjectDlg, CDialog)
...
BOOL CConvertOldProjectDlg::OnInitDialog(){
...
// CConvertOldProjectDlg message handlers

void CConvertOldProjectDlg::OnBnClickedOldPrjFilebtn()
{
	UpdateData(TRUE);
	static char BASED_CODE szFilter[] = "Label Files (*.sip)|*.sip|All Files (*.*)|*.*||";
...
void CConvertOldProjectDlg::OnBnClickedOk()
{
 	UpdateData(TRUE);
...
void CConvertOldProjectDlg::OnBnClickedOlddirbtn()
{
...
void CConvertOldProjectDlg::OnBnClickedNewdirbtn()
{
	// TODO: Add your control notification handler code here
	UpdateData(true);
...
void CConvertOldProjectDlg::OnBnClickedCancel()
{
	// TODO: Add your control notification handler code here
	OnCancel();
}
...
bool CConvertOldProjectDlg::ConvertSoLIM2Project(CString projectFile,CString newProjectName, CString newProjectDir)
{
	...
	else
	{
		...
		ifstream fin;
		fin.open(projectFile);
		if(fin.fail()){

			CString msg = "Error in opening project file:";
			msg += projectFile;
			msg += "!";
			AfxMessageBox(msg);
			return false;
		...
		if(fin.eof()){
			...
			AfxMessageBox("The format of project file is incorrect.");
			fin.close();
			return false;
		}
		...
		if(aline!="Project Name:"){
			...
			fin.getline(aline.GetBuffer(1024),1024);
			aline.ReleaseBuffer(-1);
			//this->m_pProject->setProjectName(aline);
			...
			fin.getline(aline.GetBuffer(1024),1024);
			aline.ReleaseBuffer(-1);
			/////////get current knowledgebase type

			int knowledgebaseType = 0;
			if(aline!="Current Knowledgebase Type:"){
			...
			fin.getline(aline.GetBuffer(1024),1024);
			aline.ReleaseBuffer(-1);
			if(aline == "rulebase") knowledgebaseType=0;
			else if(aline == "point casebase") knowledgebaseType=1;
			else if(aline == "line casebase")  knowledgebaseType=2;
			else if(aline == "polygon casebase") knowledgebaseType=3;
			else if(aline == "raster casebase")  knowledgebaseType=4;
			else if(aline == "null"){
				knowledgebaseType=0;				
			...
			if(knowledgebaseType==0){
				...
				fin.getline(aline.GetBuffer(1024),1024);
				aline.ReleaseBuffer(-1);		        
				CString RuleBasePath = strProjectDir+"\\"+aline;		        
				...
				if(dbf->openDBF(RuleBasePath, false)!=0 || dbf->readDBF(theAttrKnowledge)==0){
					delete dbf;
			...
			//////load point casebase////////////////////////////////////////////
				...
				fin.getline(aline.GetBuffer(1024),1024);
				aline.ReleaseBuffer(-1);		        
				CString PointCasebasePath = strProjectDir+"\\"+aline;
...
void CConvertOldProjectDlg::WriteProject(CString strFileName, CProject *pProject)
{
