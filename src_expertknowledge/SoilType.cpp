...
void CSoilType::getInferenceResult(float *resultBuf, CEnvDatabase *pEnvDatabase,bool bUseMask, bool *maskBuf, bool bCreateCheckFile, float *chkBuf,CProgressCtrl *ProgressBar, int iCurSoilID,int iSoilNum)
{
	float nodata = -9999;
		...
		for(int i=0;i<iRowNum;i++)
		{
		    ...
		    while(PeekMessage(&msg,NULL,0,0,PM_REMOVE))
           {
			...
			for(int j=0;j<iColNum;j++)
			{
				...
				dX=pEnvDatabase->getLayer(0)->m_pTheData->xOfACell(j);
				dY=pEnvDatabase->getLayer(0)->m_pTheData->yOfACell(i);
				dResultSim = 0;
				for(int k=0;k<m_iInstanceNum;k++)
				{
