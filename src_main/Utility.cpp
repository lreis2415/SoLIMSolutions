...
void HistogramEqualize(int *sourceBuf, int *targetBuf, int width, int height)
{
  ...
  if(surface == 0)
	  return;
   int nGreyValues = 251; //we only use 251 grey scale
   std::vector<int> histogram = GetHistogram(sourceBuf, width, height);
  ...
  int min = (double)(nGreyValues) * (double)(cumulativeHistogram[0]) / (double)(surface)-1;



  for (int i=0; i<nGreyValues; ++i)
  {
    rescaledHistogram[i] = ((double)(nGreyValues) * (double)(cumulativeHistogram[i]) / (double)(surface)-1-min)/(250-min)*250;
  }
  ...
  for (int y=0; y!=height; ++y)
  {
     ...
     for (int x=0; x!=width; ++x)
    {
         int greyOriginal   = sourceBuf[y*width + x];
		 if(greyOriginal != -9999)
		 {
            int greyNew = rescaledHistogram[greyOriginal];

	       targetBuf[y*width + x] = greyNew;

		 }
		 else 
		 {
			  targetBuf[y*width + x] = -9999;
		 }
...
std::vector<int> GetHistogram(int *dataBuf, int width, int height)
{
  ...
  std::vector<int> histogram(251,0); 
  for (int y=0; y!=height; ++y)
  {
    for (int x=0; x!=width; ++x)
    {	   
      int greyVal = dataBuf[y*width + x];
	  if(greyVal != -9999)
         ++(histogram[greyVal]);
    }
  }
...
void DrawAntialiasLine( CDC *pDC, int X0, int Y0, int X1, int Y1, COLORREF clrLine )
{
...
double CrossArea(vector<Point>& func1, vector<Point>& func2)	{
	...
	size_t iFunc1Size = func1.size();
	size_t iFunc2Size = func2.size();
...
string getCoreFilename(string& fullFileName)
{
	string::size_type i1 = fullFileName.find_last_of("\\");
	string::size_type i2 = fullFileName.find_last_of(".");
	return fullFileName.substr(i1+1, i2-i1-1);
}

void UTF8ToGB(string &szOut)
{
	WCHAR *strSrc;
	TCHAR *szRes;
	//长度
...
// Convert from ThreeDR to GDAL file ( added by Zhanglei )
bool Convert(CThreeDR *input, char *outputFilename, char *outputType)
{
	// get the basic info from source
	int width = input->getNumberOfColumns();
	int height = input->getNumberOfRows();
	char *dataUnits = input->getDataUnits();
	int nBands = 1;
