...
void CBackDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_TerrainPartitionFileFld, m_TPFileName);
...
BEGIN_MESSAGE_MAP(CBackDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_STN_CLICKED(IDC_TopPcntLbl, OnStnClickedToppcntlbl)
END_MESSAGE_MAP()
...
void CBackDlg::OnTerrainPartitionFileBtn(){
	...
	if(m_ldFile.DoModal() == IDOK){
		m_TPFileName = m_ldFile.GetPathName();
		UpdateData(FALSE);
	}		
}

void CBackDlg::OnRidgeFileBtn(){
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);	
	static char BASED_CODE szFilter[] = "3dr Files (*.3dr)|*.3dr|All Files (*.*)|*.*||";
 	CFileDialog m_ldFile(TRUE, NULL, "*.3dr", OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
...
void CBackDlg::OnOutputFileNameBtn(){
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);	
	static char BASED_CODE szFilter[] = "3dr Files (*.3dr)|*.3dr|All Files (*.*)|*.*||";
...
void CBackDlg::OnOK(){
	...
	int readFlag = theTerrainPartition->readin(m_TPFileName.GetBuffer(1), 0);
	if(readFlag != 0){
	...
	int opFlag = back(terrainPartition, slope, ridge, outBuf, m_TopPercent, nrow, ncol, nodata);

	if(opFlag != 0){
		MessageBox("Error in back function.");
		GetDlgItem(IDOK)->EnableWindow(TRUE);
...
void CBackDlg::OnStnClickedToppcntlbl()
{
	// TODO: 在此添加控件通知处理程序代码
}
