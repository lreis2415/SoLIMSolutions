...
{
	// TODO: Add your control notification handler code here
	/*if(nCurrentPage==4&&m_bAttachLayer==true)
	{
		nCurrentPage=2;
		ShowPage(2);
	}
...
void CRangeWizard::OnFinish() 
{
	...
	else if(m_bAttachLayer==true)
	{
		...
		else
		{
			if(m_pRule->getCurveType()==0)
			{
				m_pRule->setLowCross(pLayer->m_pTheData->getDataMean()-1.33*pLayer->m_pTheData->getDataStd());
				m_pRule->setLowUnity(pLayer->m_pTheData->getDataMean());
				m_pRule->setHighUnity(pLayer->m_pTheData->getDataMean());
				m_pRule->setHighCross(pLayer->m_pTheData->getDataMean()+1.33*pLayer->m_pTheData->getDataStd());
