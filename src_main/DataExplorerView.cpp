...
void CDataExplorerView::OnDraw(CDC* pDC)
{
	...
	else if(pDoc->m_iProjectType == 1)
	{
		...
		int iCurCategory = pApp->m_iCurCategory;

		int iCurrEnvLayerIndex=pApp->m_iCurrEnvLayerIndex;
		if(pApp->pCurLayer!=NULL)
		{

			m_iRows = pApp->pCurLayer->getRow();
			m_iCols = pApp->pCurLayer->getCol();
			DrawDataLayer(pApp->pCurLayer);
			/*
			if(iCurCategory == 0) //climate
			{
			
		      m_iRows=pDoc->m_pProjectCaseEdit->m_pEnvDatabase->getLayer(pApp->m_iCurrEnvLayerIndex)->getRow();
		      m_iCols=pDoc->m_pProjectCaseEdit->m_pEnvDatabase->getLayer(pApp->m_iCurrEnvLayerIndex)->getCol();
			  DrawDataLayer(pApp->pCurLayer);
			}
			else if(iCurCategory == 1) //geology
			{
			...
			else if(iCurCategory == 2) //terrain
			{
		      ...
		      m_iCols=pDoc->m_pProjectCaseEdit->m_pEnvDatabase->getLayer(pApp->m_iCurrEnvLayerIndex)->getCol();
			   DrawDataLayer(pApp->pCurLayer);
				...
				else if(iCurCategory == 3) //vegetation
			{
		   ...
		   //DrawDataLayer(iCurCategory, iCurrEnvLayerIndex);*/
		}
	}
...
void CDataExplorerView::DrawDataLayer(CEnvLayer *pLayer)
{
	CSoLIMApp* pApp=(CSoLIMApp *)AfxGetApp();

	CSoLIMDoc* pDoc = (CSoLIMDoc *)GetDocument();
	float fMaxValue=pLayer->m_pTheData->getDataMax();
	float fMinValue=pLayer->m_pTheData->getDataMin();
	double dRange=fMaxValue-fMinValue;
/*
	int width = pLayer->m_pTheData->getNumberOfColumns();
	int height = pLayer->m_pTheData->getNumberOfRows();

	float *pEnhancedBuf = new float(width * height);
	HistogramEqualize(pLayer->m_pTheData->getDataBuf(), pEnhancedBuf, width, height);*/
	...
	for(int i=m_iRows-1;i>=0;i--)
		...
		{
			if( fabs(pLayer->m_pTheData->getDataBuf()[i*width+j] - pLayer->m_pTheData->getNullDataValue()) < VERYSMALL)
				
				origBuf[i*width+j] = -9999;//no data;
			else
				origBuf[i*width+j] = (pLayer->m_pTheData->getDataBuf()[i*width+j] - fMinValue)/dRange * 250;
     ...
     for(int i=m_iRows-1;i>=0;i--)
	 {
		...
		for(int j=0;j<m_iCols;j++)
		{
			if(pEnhancedBuf[i*width+j] == -9999)
				...
				//image[(m_iRows-i-1)*iTempCol+j]= (GLubyte)(origBuf[i*width+j]);
		}
	 }
...
void CDataExplorerView::DrawDataLayer(int iIndex)
{
	...
	int width = pDoc->m_pProjectEdit->m_pEnvDatabase->getLayer(iIndex)->m_pTheData->getNumberOfColumns();
	int height = pDoc->m_pProjectEdit->m_pEnvDatabase->getLayer(iIndex)->m_pTheData->getNumberOfRows();
	...
	for(int i=m_iRows-1;i>=0;i--)
		for(int j=0;j<m_iCols;j++)
		{
			if( fabs(pDoc->m_pProjectEdit->m_pEnvDatabase->getLayer(iIndex)->m_pTheData->getDataBuf()[i*width+j] -  pDoc->m_pProjectEdit->m_pEnvDatabase->getLayer(iIndex)->m_pTheData->getNullDataValue()) < VERYSMALL)
				
				origBuf[i*width+j] = -9999;//no data;
			else
				origBuf[i*width+j] = (pDoc->m_pProjectEdit->m_pEnvDatabase->getLayer(iIndex)->m_pTheData->getDataBuf()[i*width+j] - fMinValue)/dRange * 250;
...
void CDataExplorerView::OnMouseMove(UINT nFlags,CPoint point)
{
	...
	if(pApp->m_iCurrEnvLayerIndex!=-1 && point.x>0)
	{
		...
		CPixelInfoView *pPixelInfoView = (CPixelInfoView *)(pInfoFrameView->m_wndSplitter.GetPane(0,1));	
	...
	{
		if(pApp->pCurLayer!=NULL && point.x>0)
	{
		int iRow=pApp->pCurLayer->getRow();
		int iCol=pApp->pCurLayer->getCol();
		CRect rectClientRect;
		GetClientRect(rectClientRect);
		pApp->m_ptBufDataIndex.y=(point.y-m_ptImgTopLeftPt.y)/m_dZoom;
		pApp->m_ptBufDataIndex.x=(point.x-m_ptImgTopLeftPt.x)/m_dZoom;
