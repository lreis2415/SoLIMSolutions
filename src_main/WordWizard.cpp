...
IMPLEMENT_DYNAMIC(CWordWizard, CDialog)
CWordWizard::CWordWizard(CWnd* pParent /*=NULL*/)
: CDialog(CWordWizard::IDD, pParent)
{
	pPageLink=NULL;
	nPageCount=0;
	nCurrentPage=0;
...
CWordWizard::~CWordWizard()
{
}

void CWordWizard::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CWordWizard, CDialog)
...
void CWordWizard::AddPage(CDialog* pDialog, UINT nID)
{
...
void CWordWizard::OnPrev() 
{	
	ShowPage(--nCurrentPage);
}

void CWordWizard::OnNext() 
{	
	if(nCurrentPage == 1)
	{
...
void CWordWizard::OnFinish() 
{
	
		m_pWordLibDlg->UpdateData(true);

		CString str;
		m_pWordLibDlg->m_comboCurveName.GetWindowText(str);

		if(str=="")
		{
			MessageBox("You must specify a phrase.");
			return;
		}
...
void CWordWizard::ShowPage(UINT nPos)
{
	struct PAGELINK* pTemp=pPageLink;
	while(pTemp)
	{
...
void CWordWizard::SetWizButton(UINT uFlag)
{
...
BOOL CWordWizard::OnInitDialog() 
{
	...
	m_pLayerDlg = new CWizardChooseLayer(2);
	m_pWordLibDlg = new CWizardWordLib();
	m_pNameDlg= new CWizardRuleName();
	m_pWordLibDlg->pWordRule = m_pRule;
	AddPage(m_pNameDlg, IDD_WIZARD_RULE_NAME);
	AddPage(m_pLayerDlg , IDD_WIZARD_CHOOSE_LAYER);
	AddPage(m_pWordLibDlg, IDD_WIZARD_WORD_LIB);
